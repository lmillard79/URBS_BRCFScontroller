...
def out_ts(aep_list,data_file,out_plot,runtype):
    ...
    for aa,aep in enumerate(aep_list):
        ...
        if aep == '100':
            plot_figs = utl.get_plot_figs(aep,'dm_ts_mr6')
            dur = '012'
...
# ==============================================================================
    # Series of switche
    ts_plot_out = True  # Output timeseries plots of the event selection.
    long_plot_out = True    # Output the long sections

 # ==============================================================================
    if ts_plot_out:
        out_ts(aep_list,os.path.join(data_loc,'DM_B15ts.pkl'),out_plot,'Design')
    ...
    if long_plot_out:
        ...
        pkl_file.close()
        res_files = ts_data['all_res_files']
        lp_all.long_section('Bris',res_files, 'Sheet 43 - Brisbane River Longitudinal Profiles Maximums - All AEPs (MR5, Plot 45)',False,aep_list)
        lp_all.long_section('BremLock',res_files,'Sheet 44 - Bremer Lockyer Longitudinal Profiles Maximums - All AEPs (MR5, Plot 57)',False,aep_list)
