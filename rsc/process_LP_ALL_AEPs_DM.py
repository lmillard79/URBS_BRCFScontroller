...
def long_sub(fig,ax,xlim,ylim,loc_range,yticks,in_files,fig_t_text,output,rp_loc,aep,obs_fpath,base_res_dir,cal_runid,base_mc_dir,mc_runid,events,mc_events,fig_title,outdir,fig_fname):
    ...
    for i, event in enumerate(events):
        year=events[i][1:]
        res_dir = os.path.join(base_res_dir,event)
        #FM modelled
...
##                       del LP1[i].LP_Mod[j]
    ...
    for aa, aep in enumerate(aep):

		# Skip 2 yr for lockyer.
        if aep == '2' and fig_t_text == 'Lockyer':
			continue

        mc_evts = mc_events[aa]
        ...
        #year=events[i][1:]
            ...
            #FM modelled
            infpath = os.path.join(base_mc_dir,(dm_file + in_files[4]))
            ...
            if os.path.isfile(infpath):
                ...
                # If 2 yr in mid-brisbane, don't plot upstream of tidal limit
                if aep == '2' and fig_t_text == 'Mid Brisbane':
                    mask = tmp_dist <= 62000
                    tmp_dist[mask] = npy.nan
                    tmp_mod[mask] = npy.nan
        ...
        for i, dat in enumerate(MC1a):
            ...
            max_array[:,i] = MC1[i].LP_Mod
            dist_array[:,i] = MC1[i].LP_dist
        ...
        ax.plot(dist_array,max_ev, color=dur_col_dict[str(aa)],linewidth=3,label='MC_Max_'+ aep)
...
def long_section(loc,mc_events,fig_title,spliced,aep):

    #locations = ['bris',' ']
    #mc_events = list(mc_events[0])
    ...
    base_res_dir = r'B:\B20702 BRCFS Hydraulics\50_Hydraulic_Models\000_General\000_Scripts\PostProcessing\DM2\calib_data\LP\DM'
    base_mc_dir = r'B:\B20702 BRCFS Hydraulics\50_Hydraulic_Models\600_D_Design_Events\TUFLOW\D\results\605\B15\long_sections'
   ...
   # Need to add a list here of the gauge file, bridge deck file,fm res
    ...
    if loc.upper()=='BRIS':
        ...
        for i in range(len(ax_both)):
            ...
            long_sub(fig,ax_both[i],xlim,ylims[i],loc_range[i],yticks[i],in_files,fig_t_text[i],output,rp_loc,aep,obs_fpath,base_res_dir,cal_runid,base_mc_dir,mc_runid,events,mc_events,fig_title,outdir,fig_fname)
            output = True
        ...
        for i in range(len(ax_both),len(ax_both)+len(ax_both)):    # Start 2 after the previous run through
            in_files = [gfile[i],brd_file[i],bed_file[i],fm_cal_file[i],fm_mc_file[i]]
            long_sub(fig,ax_both[i-2],xlim,ylims[i],loc_range[i],yticks[i],in_files,fig_t_text[i],output,rp_loc,aep,obs_fpath,base_res_dir,cal_runid,base_mc_dir,mc_runid,events,mc_events,fig_title,outdir,fig_fname)
            output = True
